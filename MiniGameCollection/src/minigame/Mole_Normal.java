...
public class Mole_Normal extends JFrame implements Runnable, ActionListener {
		...
		public Mole_Normal() {
			...
			random = new Random();
			thread_NORMAL = new Thread(this);
			thread_NORMAL.start(); // thread_NORMAL START
			System.out.println("New // 쓰레드상태 : "+ thread_NORMAL.getState() + " 쓰레드 이름 : " + thread_NORMAL.getName() + " 쓰레드 ID : " + thread_NORMAL.getId());
			...
			// North - Center Label
			mole.label_e.setText("                           MoleCatching - Stage : Normal                                ");
		...
		public void actionPerformed(ActionEvent e) {
				...
				if(time < 1) { // TimeOver
					...
					while(dialogException) {
						...
						if(check == null) {
							...
							saveMoleInfo(checked, moleScore, "Normal"); 
						}
			...
			if(obj == mole.button[9]) { // NEW START BUTTON
				if(!stateT) {	
					stateT = true;
					// 쓰레드 새로 받기
					System.out.println("Old // 쓰레드상태 : "+ thread_NORMAL.getState() + " 쓰레드 이름 : " + thread_NORMAL.getName() + " 쓰레드 ID : " + thread_NORMAL.getId());
					Thread thread_NORMAL = new Thread(this);	
					thread_NORMAL.start();
					System.out.println("New // 쓰레드상태 : "+ thread_NORMAL.getState() + " 쓰레드 이름 : " + thread_NORMAL.getName() + " 쓰레드 ID : " + thread_NORMAL.getId());
				}
					for(int i = 0; i < 9; i++) {
		...
		public void run() {
			...
			while (true) {
				...
				try {
					...
					probability1_1 = random.nextInt(2); // GoldHairMole Probability 1/2
					probability2_1 = random.nextInt(4); // MinusMouse Probability 1/4
