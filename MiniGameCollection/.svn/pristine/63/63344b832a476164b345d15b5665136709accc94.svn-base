...
public class Mole_Hard extends JFrame implements Runnable, ActionListener {
	...
	public Mole_Hard() {
		...
		random = new Random();
		thread_HARD = new Thread(this);
		thread_HARD.start(); // thread_HARD START
		System.out.println("New // 쓰레드상태 : "+ thread_HARD.getState() + " 쓰레드 이름 : " + thread_HARD.getName() + " 쓰레드 ID : " + thread_HARD.getId());
		...
		// North - Center Label
		mole.label_e.setText("                           MoleCatching - Stage : Hard                                ");
	...
	public void actionPerformed(ActionEvent e) {
			...
			if(time < 1) { // TimeOver
				...
				while(dialogException) {
					...
					if(check == null) {
						...
						saveMoleInfo(checked, moleScore, "Hard"); 
					}
		...
		if(obj == mole.button[9]) { // NEW START BUTTON
			if(!stateT) {	
				stateT = true;
				// 쓰레드 새로 받기
				System.out.println("Old // 쓰레드상태 : "+ thread_HARD.getState() + " 쓰레드 이름 : " + thread_HARD.getName() + " 쓰레드 ID : " + thread_HARD.getId());
				Thread thread_HARD = new Thread(this);	
				thread_HARD.start();
				System.out.println("New // 쓰레드상태 : "+ thread_HARD.getState() + " 쓰레드 이름 : " + thread_HARD.getName() + " 쓰레드 ID : " + thread_HARD.getId());
			}
				for(int i = 0; i < 9; i++) {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				probability1_1 = random.nextInt(3); // GoldHairMole Probability 1/3
				probability2_1 = random.nextInt(2); // MinusMouse Probability 1/2
